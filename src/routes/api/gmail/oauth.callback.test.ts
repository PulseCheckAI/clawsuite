// Tests for the Gmail OAuth2 callback handler. Mirrors the Postiz/HubSpot
// callback tests (same security contract): HMAC state-verification runs BEFORE
// the access_denied branch; token-exchange failures redirect with FIXED error
// codes only (never echo upstream bodies); the state cookie is cleared on every
...
vi.mock('@/server/gmail-oauth-store', () => ({
...
const CLIENT_ID = 'gmail_test_client_id'
const CLIENT_SECRET = 'gmail_test_client_secret'
const REDIRECT_URI = 'https://localhost:3010/api/gmail/oauth/callback'
...
function makeStateCookie(state: string, secret: string = SECRET): string {
  ...
  return `gmail_oauth_state=${state}.${mac}`
}

function callbackUrl(params: Record<string, string>): string {
  const u = new URL('http://localhost/api/gmail/oauth/callback')
...
describe('state validation ordering — runs before access_denied', () => {
  it('redirects to state_mismatch when cookie missing AND ?error=access_denied', async () => {
    ...
    expect(locationPath(response)).toBe('/gmail')
...
describe('env validation', () => {
  it('returns 500 when no HMAC secret is configured', async () => {
    delete process.env.GMAIL_STATE_SECRET
    ...
    const body = (await response.json()) as { ok: boolean; error: string }
    expect(body.error).toMatch(/HMAC|GMAIL_STATE_SECRET|CLAWSUITE_PASSWORD/)
  })
  ...
  it('returns 500 when client creds are missing', async () => {
    ...
    expect(response.status).toBe(500)
    const body = (await response.json()) as { ok: boolean; error: string }
    expect(body.error).toContain('GMAIL_CLIENT_SECRET')
  })
})

// ── 6. Token exchange — success path ─────────────────────────────────────────
describe('token exchange — success', () => {
  it('writes token (access+refresh+expires_at+scope) and redirects to /gmail?connected=1', async () => {
