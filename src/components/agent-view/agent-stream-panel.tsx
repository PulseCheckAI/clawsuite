  ...
  return (
    <>
      <button
        ...
        onClick={onClose}
      />
      <aside className="fixed inset-x-0 bottom-0 z-50 h-[70vh] rounded-t-2xl border-t border-primary-200 bg-white shadow-2xl dark:border-primary-800 dark:bg-primary-900 md:right-0 md:bottom-0 md:left-auto md:top-[var(--titlebar-h,0px)] md:h-auto md:w-[400px] md:rounded-none md:border-t-0 md:border-l">
        <div className="flex h-full min-h-0 flex-col">
          <div className="sticky top-0 z-10 border-b border-primary-200 bg-white/95 backdrop-blur dark:border-primary-800 dark:bg-primary-900/95">
            <div className="mx-auto mt-2 h-1 w-10 rounded-full bg-primary-300 dark:bg-primary-700 md:hidden" />
            <div className="flex items-start justify-between gap-3 px-4 py-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span
                    className={`h-2.5 w-2.5 rounded-full ${AGENT_COLOR_DOT_CLASS[agentColor] ?? 'bg-primary-400'}`}
                  />
                ...
                <p className="mt-1 truncate text-xs text-primary-500 dark:text-primary-400">
                  {model} · {sessionKey}
                </p>
              </div>
              <button
                type="button"
                onClick={onClose}
                className="rounded-lg px-2 py-1 text-sm text-primary-500 hover:bg-primary-100 dark:text-primary-300 dark:hover:bg-primary-800"
            ...
            <div className="flex flex-wrap gap-2 px-4 pb-3">
              <span className="inline-flex items-center gap-1 rounded-lg bg-primary-100 px-2 py-1 text-xs dark:bg-primary-800">
          ...
          >
            {messages.length === 0 ? (
              ...
              messages.map((message) => (
                <div
                  key={message.id}
                  className="rounded-lg border border-primary-200 bg-white p-2 dark:border-primary-800 dark:bg-primary-900/40"
                >
                  ...
                  <p
                    className={`whitespace-pre-wrap text-xs text-primary-800 dark:text-primary-200 ${message.role === 'tool' ? 'font-mono' : ''}`}
                  >
                    {message.text}
                  </p>
                </div>
          ...
          <div className="sticky bottom-0 border-t border-primary-200 bg-white/95 px-3 py-3 backdrop-blur dark:border-primary-800 dark:bg-primary-900/95">
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => setSteerOpen(true)}
                className="rounded-lg border border-primary-200 px-2 py-2 text-xs font-medium text-primary-800 hover:bg-primary-100 dark:border-primary-700 dark:text-primary-100 dark:hover:bg-primary-800"
              >
                Steer
              </button>
              <div className="relative">
                {menuOpen ? (
                  <div className="absolute bottom-full left-0 mb-2 w-full rounded-lg border border-primary-200 bg-white p-1 shadow-lg dark:border-primary-700 dark:bg-primary-900">
                    <button
                      type="button"
                      onClick={() => void onPauseToggle()}
                      disabled={pausePending}
                      className="flex w-full rounded-md px-2 py-1.5 text-left text-xs text-primary-700 hover:bg-primary-100 disabled:opacity-60 dark:text-primary-200 dark:hover:bg-primary-800"
                    >
                      {pausePending
                        ? 'Updating...'
                        : isPaused
                ...
                <button
                  ...
                  className="w-full rounded-lg border border-primary-200 px-2 py-2 text-xs font-medium text-primary-800 hover:bg-primary-100 dark:border-primary-700 dark:text-primary-100 dark:hover:bg-primary-800"
                >
                  Pause/Kill
                </button>
              </div>
