...
export function MissionTimeline({
  ...
  const missionCardCls = 'relative overflow-hidden rounded-2xl border border-primary-200 bg-white p-4 shadow-sm'
    ...
    <section className={cn('mx-auto w-full max-w-[960px]', missionCardCls)}>
      ...
      <ol className="space-y-4">
        <li className="flex items-start gap-3">
          ...
          <div>
            <p className="text-[16px] font-bold text-primary-900">Mission started</p>
            <p className="text-xs text-primary-500">{new Date(startedAt).toLocaleString()}</p>
          </div>
        </li>
            ...
            return (
              <li key={`dispatch-${task.id}`} className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="break-words text-base font-bold text-primary-900">
                    Agent dispatched: {member?.name ?? task.agentId}
                  </p>
                  <p className="break-words text-sm text-primary-500">
        ...
        {teamMembers.map((member) => {
          ...
          return (
            <li key={`agent-${member.id}`} className="flex items-start gap-3">
              <span className={cn('mt-2 h-3 w-3 rounded-full', isActive ? 'bg-emerald-500' : 'bg-primary-300')} />
              <div className={cn('min-w-0 flex-1', missionCardCls)}>
                <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-accent-500 via-accent-400/40 to-transparent" />
                <div className="flex items-center justify-between gap-2">
                  <p className="truncate text-base font-bold text-primary-900">{member.name}</p>
                  {isActive ? (
                    <span className="rounded-full bg-emerald-700 px-2.5 py-1 text-xs text-white">Active</span>
                  ) : (
                    <span className="text-sm text-primary-400">Stopped</span>
                  )}
                </div>
                <p className="mt-1 text-sm text-primary-500">Assigned tasks: {assignedTaskCount}</p>
                ...
                {isExpanded ? (
                  agentSessionMap?.[member.id] ? (
                    ...
                    <div className="mt-2 rounded-lg border border-primary-200 bg-white p-2">
                      <p className="text-[11px] text-primary-400">Waiting for agent session...</p>
                    </div>
                  )
                ) : null}
              </div>
        ...
        {hasMissionCompleted ? (
          <li className="flex items-start gap-3">
            <span className="mt-1 h-3 w-3 rounded-full bg-primary-300" />
            <div>
              <p className="text-base font-bold text-primary-900">Mission stopped</p>
