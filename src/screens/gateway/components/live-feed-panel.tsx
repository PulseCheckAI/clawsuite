...
const EVENT_BADGE: Record<FeedEventType, EventBadge> = {
  mission_started: { label: 'MISSION', className: 'bg-accent-950/70 text-accent-400 border border-accent-800/50' },
  ...
  agent_idle:      { label: 'IDLE',    className: 'bg-primary-100 text-primary-500 border border-primary-200 dark:bg-primary-800 dark:border-primary-700' },
  ...
  gateway_health:  { label: 'SYS',     className: 'bg-primary-100 text-primary-500 border border-primary-200 dark:bg-primary-900 dark:text-primary-600 dark:border-primary-800' },
  system:          { label: 'SYS',     className: 'bg-primary-100 text-primary-500 border border-primary-200 dark:bg-primary-900 dark:text-primary-600 dark:border-primary-800' },
}
...
function severityClass(severity: EventSeverity): string {
  ...
  return 'border-primary-200 bg-primary-50 dark:border-primary-800/60 dark:bg-primary-900/40'
}
...
function severityTextClass(severity: EventSeverity): string {
  ...
  if (severity === 'system') return 'text-primary-700 dark:text-primary-300'
...
export function LiveFeedPanel() {
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-primary-950">
      {/* ── Header ──────────────────────────────────────────────────────── */}
      <div className="flex shrink-0 items-center justify-between border-b border-primary-200 px-4 py-2.5 dark:border-primary-800">
        ...
        <div className="flex items-center gap-2">
          {events.length > 0 ? (
            <button
              type="button"
              onClick={() => setEvents([])}
              className="rounded px-1.5 py-0.5 text-[10px] text-primary-600 transition-colors hover:bg-primary-100 hover:text-primary-700 dark:hover:bg-primary-800 dark:hover:text-primary-300"
            >
              Clear
            </button>
          ) : null}
      ...
      {/* ── Filters ─────────────────────────────────────────────────────── */}
      <div className="flex shrink-0 gap-0.5 border-b border-primary-200 px-2 py-1.5 dark:border-primary-800">
        {FILTERS.map((tab) => (
          <button
            key={tab}
            type="button"
            ...
            className={cn(
              ...
              activeFilter === tab
                ...
                : 'text-primary-500 hover:bg-primary-100 hover:text-primary-800 dark:text-primary-600 dark:hover:bg-primary-900 dark:hover:text-primary-300',
            )}
          >
            {tab}
          </button>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        {/* Top fade overlay */}
        <div className="pointer-events-none absolute inset-x-0 top-0 z-10 h-8 bg-gradient-to-b from-white to-transparent dark:from-primary-950" />

        <div className="h-full overflow-y-auto px-3 pb-3 pt-8">
          {visibleEvents.length === 0 ? (
            <div className="flex min-h-[220px] items-center justify-center">
              <p className="text-center font-mono text-[10px] text-primary-600 dark:text-primary-500">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {event.agentName ? (
                        <p className="mt-0.5 truncate font-mono text-[9px] text-primary-600 dark:text-primary-500" title={event.agentName}>
                          {event.agentName}
                        </p>
                      ) : null}
                    </div>
