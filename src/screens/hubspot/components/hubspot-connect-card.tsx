// HubSpot module — OAuth connect/disconnect card. Mirrors the Postiz connect
// card. Reads /api/hubspot/oauth/status on mount, surfaces ?connected=1 /
// ?error=<x> banners after the round-trip, then strips the query.
//
// Backend contract:
//   GET    /api/hubspot/oauth/start    → 302 to HubSpot authorize (use
//                                         window.location.href so the browser
//                                         follows the redirect + sets the cookie)
//   GET    /api/hubspot/oauth/callback → /hubspot?connected=1 | ?error=<code>
//   GET    /api/hubspot/oauth/status   → { connected, scope?, expiresAt?, obtainedAt? }
//   DELETE /api/hubspot/oauth          → { ok: true }
...
type StatusState =
  | { kind: 'loading' }
  | { kind: 'ok'; data: HubSpotOAuthStatus }
...
type BannerState =
  ...
  | { kind: 'access_denied' }
  | { kind: 'error'; message: string }
  | { kind: 'disconnect_failed'; message: string }
  | null
...
function parseInitialBanner(search: string): BannerState {
  ...
  if (params.get('connected') === '1') return { kind: 'success' }
...
export function HubSpotConnectCard() {
  ...
  useEffect(() => {
    ...
    if (initial) {
      setBanner(initial)
      try {
        window.history.replaceState({}, '', '/hubspot')
      } catch {
        // best-effort
      }
    }
  ...
  const refreshStatus = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const data = (await res.json()) as HubSpotOAuthStatus
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const res = await fetch('/api/hubspot/oauth', {
        method: 'DELETE',
        credentials: 'same-origin',
      })
      if (!res.ok) {
        setBanner({
          ...
          message: `Disconnect failed (HTTP ${res.status}). The connection may still be active.`,
        })
      }
    } catch (err) {
      setBanner({
      ...
      // Re-sync against the server so the UI reflects the true state whether or
          ...
          <div className="space-y-1">
            ...
            >
              <HugeiconsIcon
                ...
                aria-hidden="true"
              />
              Couldn't load HubSpot connection status
            </div>
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm" style={{ color: 'var(--mc-text-dim)' }}>
          Read contacts and deals from your HubSpot CRM.
        </p>
      </div>
      <button
        type="button"
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 flex-1">
        ...
        >
          {data.scope ? (
            <div className="truncate" title={data.scope}>
              scopes{' '}
              <code
...
function Banner({
  ...
  if (banner.kind === 'success') {
    ...
    color = 'var(--mc-rose)'
    title = 'Session expired during HubSpot authorization. Please try again.'
  } else if (banner.kind === 'access_denied') {
    icon = InformationCircleIcon
    borderColor = 'var(--mc-border-bright)'
    background = 'var(--mc-surface-2)'
    color = 'var(--mc-text-dim)'
    title = 'You declined the HubSpot authorization request.'
  } else if (banner.kind === 'disconnect_failed') {
    role = 'alert'
    icon = Alert02Icon
    borderColor = 'var(--mc-rose)'
    background = 'var(--mc-rose-soft)'
    color = 'var(--mc-rose)'
    title = "Couldn't disconnect from HubSpot."
    body = banner.message
  } else {
    role = 'alert'
    icon = Alert02Icon
