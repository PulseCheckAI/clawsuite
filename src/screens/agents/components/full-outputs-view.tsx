...
const FILTER_META: Record<AgentOutputFilter, { label: string; emoji: string }> =
  {
    all: { label: 'All', emoji: '📋' },
    success: { label: 'Success', emoji: '✅' },
    error: { label: 'Errors', emoji: '❌' },
  }
...
function OutputCard({ output }: { output: AgentOutput }) {
  ...
  const parsedTs = new Date(output.timestamp).getTime()
  const relativeTime = formatRelativeTime(
    Number.isFinite(parsedTs) ? parsedTs : Date.now(),
  )
  const duration = formatDuration(output.durationMs)
  const fullOutput = output.fullOutput ?? ''
  const sageTweet = useMemo(() => extractSageTweet(fullOutput), [fullOutput])
  const sagePrompt = useMemo(() => extractSagePrompt(fullOutput), [fullOutput])
  const sourceUrl = useMemo(() => extractFirstUrl(fullOutput), [fullOutput])
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
        <Button
          ...
          onClick={() => void copyText(fullOutput, 'Output')}
        >
        ...
        {output.status === 'error' && output.jobId ? (
          <Button
            variant="secondary"
            className="border border-[var(--theme-border)] bg-white text-[var(--theme-text)] hover:bg-[var(--theme-card2)]"
          ...
          <div className="flex flex-wrap gap-2">
            {availableFilters.map((item) => (
              <FilterPill
                key={item}
                active={filter === item}
                label={FILTER_META[item].label}
                emoji={FILTER_META[item].emoji}
