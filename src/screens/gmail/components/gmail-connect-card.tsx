// Gmail module — OAuth connect/disconnect card. Mirrors the HubSpot connect
// card. Reads /api/gmail/oauth/status on mount, surfaces ?connected=1 /
          ...
          <div className="space-y-1">
            ...
            >
              <HugeiconsIcon
                ...
                aria-hidden="true"
              />
              Couldn't load Gmail connection status
            </div>
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm" style={{ color: 'var(--mc-text-dim)' }}>
          Read your recent inbox messages (read-only scope).
        </p>
      </div>
      <button
        type="button"
...
function Banner({
  ...
  if (banner.kind === 'success') {
    ...
    color = 'var(--mc-rose)'
    title = 'Session expired during Gmail authorization. Please try again.'
  } else if (banner.kind === 'access_denied') {
    icon = InformationCircleIcon
    borderColor = 'var(--mc-border-bright)'
    background = 'var(--mc-surface-2)'
    color = 'var(--mc-text-dim)'
    title = 'You declined the Gmail authorization request.'
    ...
    background = 'var(--mc-rose-soft)'
    color = 'var(--mc-rose)'
    title = "Couldn't disconnect from Gmail."
    body = banner.message
  } else {
    role = 'alert'
    icon = Alert02Icon
