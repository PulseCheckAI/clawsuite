...
export function RecentSessionsWidget({
  ...
  return (
    <DashboardGlassCard
      ...
      icon={Clock01Icon}
      titleAccessory={
        <span className="inline-flex items-center rounded-full border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-2 py-0.5 font-mono text-[11px] font-medium text-primary-800 dark:text-primary-200 tabular-nums">
          {sessions.length} active
        </span>
      }
      draggable={draggable}
      onRemove={onRemove}
      className="h-full rounded-xl border border-primary-200 dark:border-primary-700 border-l-4 border-l-teal-500 bg-white dark:bg-primary-900 p-4 sm:p-5 shadow-[0_6px_20px_rgba(0,0,0,0.25)] [&_svg]:text-teal-500"
    >
      {isLoading ? (
        <div className="flex h-32 items-center justify-center gap-3 rounded-lg border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950">
          <span
            className="size-4 animate-spin rounded-full border-2 border-primary-300 dark:border-primary-700 border-t-primary-300"
            role="status"
            aria-label="Loading"
          />
          <span className="text-sm text-primary-500 dark:text-primary-400">Loading sessions…</span>
        </div>
      ) : sessions.length === 0 ? (
        <div className="flex h-32 flex-col items-center justify-center gap-1 rounded-lg border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950">
          <p className="text-sm font-semibold text-primary-900 dark:text-primary-100">No sessions yet</p>
          ...
          {sessions.map(function mapSession(session, index) {
            return (
              <Button
                ...
                variant="outline"
                className={cn(
                  'group h-auto w-full flex-col items-start rounded-lg border border-primary-200 dark:border-primary-800 px-3.5 py-3 text-left shadow-sm transition-all hover:-translate-y-[1px] hover:border-primary-300 dark:hover:border-primary-700',
                  index % 2 === 0
                    ? 'bg-primary-50 dark:bg-primary-950 hover:bg-primary-100 dark:hover:bg-primary-800'
                    : 'bg-primary-50/80 dark:bg-primary-950/80 hover:bg-primary-100 dark:hover:bg-primary-800/90',
                )}
              ...
              >
                <div className="flex w-full items-center justify-between gap-3">
                  <span className="line-clamp-1 text-sm font-semibold text-primary-900 dark:text-primary-100 text-balance">
                    {session.title}
                  </span>
                  <span className="flex shrink-0 items-center gap-1">
                    <span className="rounded-full border border-primary-200 dark:border-primary-800 bg-white dark:bg-primary-900 px-2 py-0.5 font-mono text-[10px] text-primary-500 dark:text-primary-400 tabular-nums">
                    ...
                    <HugeiconsIcon
                      ...
                      size={14}
                      strokeWidth={1.5}
                      className="text-primary-500 dark:text-primary-400 opacity-0 transition-opacity group-hover:opacity-100"
                    />
                  </span>
                </div>
                <p className="mt-1 line-clamp-2 w-full text-left text-sm text-primary-500 dark:text-primary-400 text-pretty">
                  {session.preview}
                </p>
              </Button>
            )
