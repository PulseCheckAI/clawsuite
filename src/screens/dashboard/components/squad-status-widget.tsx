...
const STATUS_DOT: Record<AgentStatus, string> = { active: 'bg-emerald-500', idle: 'bg-yellow-500', available: 'bg-primary-400', paused: 'bg-red-500' }
...
export function SquadStatusWidget({ editMode }: SquadStatusWidgetProps) {
  ...
  return (
    <WidgetShell
      ...
      icon={BotIcon}
      editMode={editMode}
      action={<span className="inline-flex items-center rounded-full border border-primary-200 bg-primary-100/80 px-2 py-0.5 text-[11px] font-medium text-primary-600 tabular-nums dark:border-primary-700 dark:bg-primary-800 dark:text-primary-300">{agents.length}</span>}
      className="h-full border border-primary-200 dark:border-primary-700 border-l-4 border-l-teal-500 bg-white dark:bg-primary-900"
    >
      {sessionsQuery.isLoading && agents.length === 0 ? (
        <div className="flex h-[150px] items-center justify-center gap-3 rounded-xl border border-primary-200 bg-primary-100/50 dark:border-primary-800 dark:bg-primary-900/40">
          <span className="size-4 animate-spin rounded-full border-2 border-primary-300 border-t-primary-600" />
          <span className="text-sm text-primary-600 dark:text-primary-300">Loading squad…</span>
        </div>
      ) : visibleAgents.length === 0 ? (
        <div className="flex h-[150px] flex-col items-center justify-center gap-1 rounded-xl border border-primary-200 bg-primary-100/50 text-center dark:border-primary-800 dark:bg-primary-900/40">
          <p className="text-sm font-semibold text-primary-800 dark:text-primary-100">No agents yet</p>
          <p className="text-xs text-primary-500 dark:text-primary-400">Launch or connect an agent to see live sessions here.</p>
        </div>
      ) : (
        <div className="space-y-1.5">
          {visibleAgents.map((agent) => (
            <div key={agent.id} className="flex items-center gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-primary-50 dark:hover:bg-white/10 dark:hover:bg-primary-800/50">
              <span className={cn('size-2 shrink-0 rounded-full', STATUS_DOT[agent.status])} />
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-primary-900 dark:text-primary-100">{agent.name}</p>
                <p className="truncate text-[11px] text-primary-500 dark:text-primary-400">{STATUS_LABEL[agent.status]}</p>
              </div>
              <p className="hidden max-w-[120px] truncate text-[11px] italic text-primary-500 dark:text-primary-400 sm:block">{agent.taskPreview || '—'}</p>
              <span className="shrink-0 text-[11px] text-primary-500 dark:text-primary-400 tabular-nums">{agent.timeAgo}</span>
              {agent.tokens > 0 && (
                <span className="shrink-0 text-[10px] tabular-nums text-primary-500 dark:text-primary-400">{formatTokenCompact(agent.tokens)}</span>
              )}
              <span className="shrink-0 rounded-full bg-primary-100 px-1.5 py-0.5 text-[10px] font-medium text-primary-600 dark:bg-primary-800 dark:text-primary-300">{agent.modelShort}</span>
            </div>
          ))}
          {agents.length > 6 ? (
            <button type="button" onClick={() => void navigate({ to: '/agents' })} className="mt-1 inline-flex text-[11px] font-medium text-primary-500 dark:text-primary-400 transition-colors hover:text-primary-700 dark:hover:text-primary-200">
              View all →
            </button>
          ) : null}
        </div>
