...
export function UsageMeterWidget({
  ...
  const tabSwitcher = (
    <div className="hidden items-center gap-0.5 rounded-full border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 p-0.5 text-[10px] md:inline-flex">
      {(['cost', 'tokens'] as const).map((tab) => (
        <button
          ...
          className={cn(
            ...
            view === tab
              ? 'bg-primary-100 dark:bg-primary-800 text-primary-900 dark:text-primary-100 shadow-sm'
  ...
  return (
    <WidgetShell
      ...
      className="h-full rounded-xl border border-primary-200 dark:border-primary-700 border-l-4 border-l-purple-500 bg-white dark:bg-primary-900 p-4 sm:p-5 [&_svg]:text-purple-500"
    >
      {showTimeoutOrError ? (
        <div className="flex flex-col items-start gap-2 rounded-lg border border-red-900 bg-red-950/30 px-3 py-3">
          ...
          <button
            ...
            className="rounded-md border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-2.5 py-1 text-xs font-medium text-primary-800 dark:text-primary-200 transition-colors hover:bg-primary-100 dark:hover:bg-primary-800"
          >
            Retry
          </button>
        </div>
        ...
        <div className="flex h-full flex-col items-center justify-center gap-1.5 py-4 text-center">
          <p className="text-sm font-medium text-primary-700 dark:text-primary-300">No usage yet today</p>
          ...
          {isSettled && (
            <button
              ...
              className="mt-1 rounded-md border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-2.5 py-1 text-xs font-medium text-primary-700 dark:text-primary-300 transition-colors hover:bg-primary-100 dark:hover:bg-primary-800"
            >
              Refresh
            </button>
          )}
        ...
        <>
          ...
          <div className="space-y-3 md:hidden">
            <div className="flex items-baseline gap-2">
              <p className="font-mono text-2xl sm:text-3xl font-bold leading-none tabular-nums text-primary-900 dark:text-primary-100">
                {view === 'cost' ? formatUsd(displayCost) : formatTokens(displayTokens)}
              </p>
              <span className="text-xs text-primary-500 dark:text-primary-400">{view === 'cost' ? 'today' : 'tokens'}</span>
            </div>

            <div className="inline-flex gap-1 rounded-full border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 p-1">
              {(['cost', 'tokens'] as const).map((tab) => (
                <button
                  key={tab}
                  type="button"
                ...
                {topProviders.map((p) => (
                  <div key={p.provider} className="space-y-0.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-[11px] tabular-nums text-primary-700 dark:text-primary-300">
                        {view === 'cost' ? formatUsd(p.cost) : formatTokens(p.total)}
                      </span>
                    </div>
                    <div className="h-1 w-full overflow-hidden rounded-full bg-primary-100 dark:bg-primary-800">
                      <div
          ...
          <div className="hidden space-y-3 md:block">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-2xl sm:text-3xl font-bold leading-none text-primary-900 dark:text-primary-100 tabular-nums">
                  {view === 'cost' ? formatUsd(displayCost) : formatTokens(displayTokens)}
                </p>
                <p className="mt-1 text-[11px] uppercase tracking-wider text-primary-500 dark:text-primary-400">
                  {view === 'cost' ? 'today\'s spend' : 'tokens today'}
                </p>
              </div>
              <div className="text-right">
                <p className="font-mono text-sm font-semibold tabular-nums text-primary-700 dark:text-primary-300">
                  {view === 'cost' ? formatTokens(displayTokens) : formatUsd(displayCost)}
                </p>
                <p className="mt-0.5 text-[11px] uppercase tracking-wider text-primary-500 dark:text-primary-400">
                  ...
                  <div key={p.provider} className="space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-[10px] tabular-nums text-primary-500 dark:text-primary-400">
                          {formatTokens(p.total)} tok
                        </span>
                        <span className="font-mono text-[11px] font-semibold tabular-nums text-primary-800 dark:text-primary-200">
                          {formatUsd(p.cost)}
                        </span>
                      </div>
                    </div>
