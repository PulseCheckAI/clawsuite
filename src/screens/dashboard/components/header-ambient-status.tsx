...
export function HeaderAmbientStatus() {
  ...
  return (
    <div className="hidden text-right sm:block">
      <div className="inline-flex items-center justify-end gap-2 rounded-full border border-primary-200 bg-primary-100/65 px-3 py-1 text-[11px] text-primary-600 tabular-nums shadow-sm dark:border-primary-700 dark:bg-primary-900/80 dark:text-primary-300">
        <span
        ...
        <span className="text-primary-600 dark:text-primary-300">{dateStr}</span>
        {weather ? (
          <>
            <span className="text-primary-400 dark:text-primary-500">·</span>
            <span
              ref={weatherRef}
              className="relative cursor-pointer text-primary-600 dark:text-primary-300 transition-colors hover:text-accent-600"
            ...
            >
              ...
              {showWeatherPopover && (
                <div
                  ref={popoverRef}
                  className="absolute right-0 top-full z-50 mt-2 w-64 rounded-lg border border-primary-200 bg-white p-3 shadow-lg dark:border-primary-700 dark:bg-primary-900"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="mb-2 text-xs font-medium text-ink dark:text-primary-100">
                    Weather Location
                  </div>
                  <input
                    type="text"
                    className="w-full rounded border border-primary-200 bg-white px-2 py-1.5 text-xs text-ink outline-none focus:border-accent-400 focus:ring-1 focus:ring-accent-200 dark:border-primary-700 dark:bg-primary-800 dark:text-primary-100"
                    value={locationInput}
