...
function priorityBadgeClass(priority: TaskPriority): string {
  ...
  if (priority === 'P1') return 'bg-accent-950/70 text-accent-300'
...
function statusDotClass(status: TaskStatus): string {
  if (status === 'in_progress' || status === 'review') return 'bg-accent-400'
  ...
  return (
    <>
      <WidgetShell
        ...
        icon={Task01Icon}
        action={
          <span className="inline-flex items-center rounded-full border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-2 py-0.5 text-[10px] font-medium text-primary-700 dark:text-primary-300">
            Backlog {backlogCount} • In progress {inProgressCount} • Done {doneCount}
          </span>
        }
        className="h-full rounded-xl border border-primary-200 dark:border-primary-700 border-l-4 border-l-accent-500 bg-white dark:bg-primary-900 p-4 sm:p-5 md:hidden [&_svg]:text-accent-500"
      >
        {cronJobsQuery.isLoading && tasks.length === 0 ? (
          <div className="rounded-lg border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-3 py-3 text-sm text-primary-500 dark:text-primary-400">
            Loading tasks…
          </div>
            ...
            <div className="space-y-1.5">
              {mobilePreviewTasks.map(function renderMobileTask(task) {
                return (
                  <article
                    key={task.id}
                    className="flex items-center gap-2 rounded-xl border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-3 py-2"
                  >
                    <span
                      className={cn(
                    ...
                    <span className="min-w-0 flex-1 truncate text-sm font-medium text-primary-900 dark:text-primary-100">
                      {task.title}
                    </span>
                    <span
        ...
        <div className="mt-2 flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-1 text-xs font-medium text-primary-500 dark:text-primary-400 transition-colors hover:text-primary-700 dark:hover:text-primary-300"
          >
            View all ›
          </button>
        </div>
      ...
      <div className="hidden h-full md:block">
        <DashboardGlassCard
          ...
          draggable={draggable}
          onRemove={onRemove}
          className="h-full rounded-xl border-primary-200 dark:border-primary-800 bg-white dark:bg-primary-900 p-4 sm:p-5 shadow-[0_6px_20px_rgba(0,0,0,0.25)] [&_h2]:text-[11px] [&_h2]:font-medium [&_h2]:uppercase [&_h2]:tracking-wider [&_h2]:text-primary-400 dark:[&_h2]:text-primary-500 [&_svg]:text-primary-400 dark:[&_svg]:text-primary-500"
        >
          {cronJobsQuery.isLoading && tasks.length === 0 ? (
            <div className="rounded-lg border border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-950 px-3 py-3 text-sm text-primary-500 dark:text-primary-400">
              Loading tasks…
            ...
            <div className="space-y-1.5">
              {visibleTasks.map(function renderTask(task, index) {
                return (
                  <article
                    key={task.id}
                    className={cn(
                      'flex items-center gap-2 rounded-lg border border-primary-200 dark:border-primary-800 px-2.5 py-2',
                      index % 2 === 0 ? 'bg-primary-50 dark:bg-primary-950' : 'bg-primary-50 dark:bg-primary-950/80',
                    )}
                  >
                    <span
                      className={cn('size-2 shrink-0 rounded-full', statusDotClass(task.status))}
                    />
                    <span className="min-w-0 flex-1 truncate text-sm text-primary-900 dark:text-primary-100">
              ...
              {remainingCount > 0 ? (
                <p className="px-1 text-xs text-primary-500 dark:text-primary-400">+{remainingCount} more</p>
              ) : null}
            </div>
          )}
